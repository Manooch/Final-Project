...
def plotPrediction(knn, y_pred, accuracy, folder, name):
    X = breast.data[:,0:2]
    ...
    plt.title(name + " (accuracy: " + str(round(accuracy, 3)) + ")")
    plt.xlabel(breast.feature_names[0])
    plt.ylabel(breast.feature_names[1])
    plt.axis('tight')   
    plt.savefig('./{0}/{1}_{2}_{3}.png'.format(folder, name, breast.feature_names[0], breast.feature_names[1]), bbox_inches = 'tight')
    #plt.show()
    plt.close('all')
...
def KNNPredict_BreastCancer():
    ...
    plotPrediction(clf, y_pred, accuracy, 'Figures', 'K Nearest Neighbors')
...
def GaussianPredict_BreastCancer():
    ...
    plotPrediction(clf, y_pred, accuracy, 'Figures', 'Gaussian')
...
def DecisionTree_BreastCancer():
    ...
    plotPrediction(clf, y_pred, accuracy, 'Figures', 'DecisionTree')
